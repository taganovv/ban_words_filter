...
├── installer-contents/                # Содержимое DMG (распакованный установщик)
│   ├── Ban Words Filter.app/          # Готовое self-contained приложение
│   └── Applications -> /Applications  # Симлинк для drag-and-drop установки
```

| Папка | Назначение | Размер (прибл.) |
|-------|-----------|-----------------|
| `src/` | Исходники для разработки и пересборки | ~5 МБ |
| `installer-contents/` | Распакованный DMG — готовый `.app` + симлинк | ~107 МБ |
| `tools/` | Скрипты сборки macOS-версии | ~8 КБ |
...
```bash
open "installer-contents/Ban Words Filter.app"
# или после сборки:
