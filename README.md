...
**Ban Words Filter** подключается к чату вашего канала от вашего имени — отдельный бот-аккаунт не нужен. Вы один раз вводите данные из [dev.twitch.tv](https://dev.twitch.tv/), нажимаете «Старт» — и программа работает, пока запущена (в том числе из трея).
...
| **Действия** | Автоматический **ban** через Twitch Helix API | Зависит от сервиса; не всегда мгновенный бан |
...
Зритель напишет запрещенное слово с невидимым символом `пример: п-в0рд` или пролонгацию `пример: п-воооорд` — фильтр нормализует текст и всё равно поймает совпадение. У типичных списков запрещённых слов в облачных ботах такой глубины обработки нет.

**5. Бесплатно и без подписок**  
...
- Модераторам крупных каналов, чтобы автоматизоровать ручные баны, в обход фильтров 
...
- **Убирает нарушителя из чата** — банит через Twitch API.
...
- **Старт и стоп** — включили перед стримом, выключили после. Закрытие окна сворачивает в трей — фильтр продолжает работать; полный выход — через «Выход» или меню трея.
- **Тест фильтра** — можно проверить любую фразу и увидеть, сработает ли бан, ещё до стрима.
- **Словари** — редактирование чёрного и белого списков, автоматический regex.
- **Баны** — история банов за сегодня, разбан одной кнопкой.
- **Лог** — что происходило: подключение, срабатывания, ошибки.
- **Трей** — закрытие окна сворачивает программу в трей; фильтр продолжает работать.
- **Автообновление** — при запуске проверяется GitHub Releases; обновление устанавливается через встроенный установщик.
...
Подробная пошаговая инструкция — кнопка **«Инструкция»** в приложении (открывается локально, без интернета).
...
3. **Дополнительные параметры** — чекбокс «Создать ярлык на рабочем столе» (включён по умолчанию)
4. **Установка файлов** — перед копированием завершается запущенный `BanWordsFilter.exe`; при блокировке файлов показывается понятное сообщение
5. **Завершение** — чекбоксы «Открыть программу» и «Поддержать разработчика» (оба включены по умолчанию)

Тихая установка `/S` поддерживается для автообновления.
...
Приложение построено по паттерну **MVVM** на фреймворке **Avalonia UI**.
...
┌──────────────────────────▼──────────────────────────────────┐
│  MainViewModel                                              │
│  Команды UI, статус, лог, словари, баны, тест фильтра       │
└──────────────────────────┬──────────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────────┐
│  BotLifecycleService                                        │
...
│  ├── TwitchBotService                                       │
│  ├── BannedWordsConfigService / BanHistoryService           │
│  ├── UpdateCheckService / UpdateDownloadService             │
│  ├── UpdateApplyService                                     │
│  ├── TrayService / SingleInstanceGuard                      │
│  └── InstalledAppLocator                                    │
└─────────────────────────────────────────────────────────────┘
```
...
| `TwitchBotService` | `Services/TwitchBotService.cs` | IRC-подключение к чату, обработка сообщений, ban |
...
| `UpdateCheckService` | `Services/UpdateCheckService.cs` | Проверка GitHub Releases при запуске |
| `UpdateApplyService` | `Services/UpdateApplyService.cs` | Бесшовное обновление через NSIS `/S` |
| `TrayService` | `Services/TrayService.cs` | Иконка в трее, выход из приложения |
...
| `threats_violence` | high | `timeout_or_ban` | Угрозы насилия (в приложении — ban) |
| ... | ... | ... | ... |
...
Все срабатывания приводят к **permanent ban** через Helix API.
...
| `moderator:manage:banned_users` | Ban через Helix API |
...
- Единственные внешние запросы — к API Twitch и GitHub Releases (проверка обновлений)
- Кнопка «Удалить всё» удаляет `config.json`, пользовательские словари и историю банов

---

## Пользовательский интерфейс

Avalonia UI с тёмной темой, шрифт **Inter**. Пять вкладок:

| Вкладка | Функции |
|---------|---------|
| **Настройки** | Поля Twitch API, OAuth, проверка токена, Старт/Стоп, статус |
| **Тест** | Локальная проверка фразы против фильтра |
| **Лог** | Журнал событий (обновляется каждую секунду) |
| **Словари** | Чёрный и белый списки, поиск, автоматический regex |
| **Баны** | История банов за сегодня, разбан |

Дополнительные окна:

- `InstructionsWindow` — пошаговая инструкция настройки Twitch OAuth
- `UpdateDialog` / `UpdateCheckingWindow` — проверка и установка обновлений
- `AlreadyRunningDialog` — защита от второго экземпляра
- `ConfirmExitDialog` — подтверждение выхода
- `ConfirmChatTestDialog`, `ConfirmClearAllDialog`, `ConfirmRemoveAllRegexDialog`
- `TrayNotificationWindow` — уведомление о сворачивании в трей
...
| Python 3 | Генерация ICO и PNG-иконок |
